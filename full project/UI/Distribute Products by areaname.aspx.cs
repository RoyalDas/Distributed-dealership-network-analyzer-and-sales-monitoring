...
{
    public partial class Distribute_Products_by_areaname : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                List<Product> allProducts = manager.GetAllProduct("spproductdetails", null);
        ...
        protected void SaveButton_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                ProductManager manager = new ProductManager();
                int nroweffected = manager.DistributeProducts(nProduct);
                if (nroweffected > 0)
                {
                    List<Product> allProducts = manager.UpdateProAndShow(nProduct);
                    ProductGridView.DataSource = allProducts;
                    ProductGridView.DataBind();
